...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		//Instead of selecting the device manually, automatically pick it based on the IP passed on
		Arena::DeviceInfo cameraToUse;
		bool found = false;
		for (std::size_t i = 0, n = devices.size(); i < n; i++)
		{
			Arena::DeviceInfo device = devices[i];
			std::cout << "\n" << device.IpAddressStr();

			//Hack because the IpAddressStr() returns some custom string class intead of std::string
			std::string currentIp  (device.IpAddressStr().c_str());
			if(currentIp == ip){
				std::cout << "\nFound a match!";
				cameraToUse = device;
				found = true;
		...
		if(!found)
		{
			std::cout << "\nNo camera found with given Ip adress\nPress enter to complete\n";
			return 0;
		}

		Arena::IDevice* pDevice = pSystem->CreateDevice(cameraToUse);
