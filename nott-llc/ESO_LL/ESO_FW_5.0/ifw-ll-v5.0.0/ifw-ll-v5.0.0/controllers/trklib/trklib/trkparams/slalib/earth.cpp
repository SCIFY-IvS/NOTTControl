...
/* True anomaly */
   v = em + 2.0f * e * (float) sin_ ( (double) em )
          + 1.25f * esq * (float) sin_ ( 2.0 * (double) em );

/* True ecliptic longitude */
   elt = v + gam;

/* True distance */
   r = ( 1.0f - esq ) / ( 1.0f + e * (float) cos_ ( (double) v ) );

/* Moon's mean longitude */
   elmm = (float) dmod ( ( 4.72 + 83.9971 * ( (double) t ) ), D2PI );

/* Useful functions */
   coselt = (float) cos_ ( (double) elt );
   sineps = (float) sin_ ( (double) eps0 );
   coseps = (float) cos_ ( (double) eps0 );
   w1 = -r * (float) sin_ ( (double) elt );
   w2 = -SPEED * ( coselt + e * (float) cos_ ( (double) gam ) );
   selmm = (float) sin_ ( (double) elmm );
   celmm = (float) cos_ ( (double) elmm );

/* Earth position and velocity */
   pv[0] = - r * coselt - REMB * celmm;
   pv[1] = ( w1 - REMB * selmm ) * coseps;
   pv[2] = w1 * sineps;
   pv[3] = SPEED * ( (float) sin_ ( (double) elt ) +
                 e * (float) sin_ ( (double) gam ) ) + SEMB * selmm;
