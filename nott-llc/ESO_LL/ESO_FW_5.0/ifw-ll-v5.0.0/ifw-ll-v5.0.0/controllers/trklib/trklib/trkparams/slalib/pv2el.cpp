...
/* Inclination. */
   oi = atan2_ ( sqrt_ ( hx2py2 ), hz );

/* Longitude of ascending node. */
   bigom = ( hx != 0.0 || hy != 0.0 ) ? atan2_ ( hx, -hy ) : 0.0;
...
/* Eccentricity. */
   e2 = 1.0 - ar * h2 / gmu;
   ecc = ( e2 >= 0.0 ) ? sqrt_ ( e2 ) : 0.0;

/* True anomaly. */
   s = h * rdv;
   c = h2 - r * gmu;
   at = ( s != 0.0 || c != 0.0 ) ? atan2_ ( s, c ) : 0.0;

/* Argument of the latitude. */
   s = sin_ ( bigom );
   c = cos_ ( bigom );
   u = atan2_ ( ( - x * s + y * c ) * cos_ ( oi ) + z * sin_ ( oi ),
               x * c + y * s );
...
/* Capture near-parabolic cases. */
   if ( fabs_ ( ecc - 1.0 ) < PARAB ) ecc = 1.0;
...
/* Ellipse? */
   ...
   /* Eccentric anomaly. */
      ae = 2.0 * atan2_ ( sqrt_ ( -em1 ) * shat, sqrt_ ( ep1 ) * chat );
   ...
   /* Ellipse, parabola, hyperbola? */
      ...
      /* Parabola or hyperbola: evaluate tan ( ( true anomaly ) / 2 ) */
         ...
         /* Hyperbola: epoch of perihelion. */
            thhf = sqrt_ ( em1 / ep1 ) * that;
            f = log_ ( 1.0 + thhf ) - log_ ( 1.0 - thhf );
            tp = date - ( ecc * sinh_ ( f ) - f ) / sqrt_ ( - gar3 );
         }
      }
   }
