...
double timeGetMudpiTime(LONGLONG dcTime, LONG leap_second)
{
	double tmp;

	// Convert DC time from nanoseconds to microseconds
	tmp = (double)(dcTime / 1000.0);

	// Convert time from microseconds to seconds for the MUDPI protocol
	tmp = tmp / 1000000.0;

	// Add correction between UTC and TAI
	tmp = tmp + leap_second;
...
double timeGetMudpiTimeInFuture(LONGLONG dcTime, LONG leap_second, double time_ahead)
{
	double tmp;
