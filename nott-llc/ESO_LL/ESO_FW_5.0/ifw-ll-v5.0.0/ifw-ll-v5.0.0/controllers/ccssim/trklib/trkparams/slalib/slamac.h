...
#define dint(A) (((A)<0.0)?ceil_(A):floor_(A))

/* aint(A) - truncate to nearest whole number towards zero (float) */
#define aint(A) (((A)<0.0f)?(float)ceil_((double)(A))\
                                             :(float)floor_((double)(A)))

/* dnint(A) - round to nearest whole number (double) */
#define dnint(A) (((A)<0.0)?ceil_((A)-0.5):floor_((A)+0.5))
...
#define dmod(A,B) (((B)!=0.0)?((A)*(B)>0.0?(A)-(B)*floor_((A)/(B))\
                                          :(A)+(B)*floor_(-(A)/(B))):(A))
