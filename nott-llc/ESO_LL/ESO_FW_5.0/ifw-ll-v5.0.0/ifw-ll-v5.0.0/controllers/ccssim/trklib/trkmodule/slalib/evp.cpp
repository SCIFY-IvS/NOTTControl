...
/* Elliptic part of the motion of the EMB */
   ...
   g = forbel[0];
   twog = g + g;
   phi = twoe * ( ( 1.0 - esq / 8.0 ) * sin_ ( g )
                + 5.0 * e * sin_ ( twog ) / 8.0
                + 13.0 * esq * sin_ ( g + twog ) / 24.0 );
   f = forbel[0] + phi;
   sf = sin_ ( f );
   cf = cos_ ( f );
...
/* Heliocentric motion of the Earth */
   ...
   dzhd  = - sigma * ccfdi * cos_ ( forbel[2] );
...
/* Barycentric motion of the Earth */
   ...
   for ( k = 0; k < 4; k++ ) {
      ...
      pomg = sorbel[k+1];
      pecc = sorbel[k+9];
      tl = dmod( plon + 2.0 * pecc * sin_ ( plon - pomg ) , D2PI );
      sinlp[k] = sin_ ( tl );
      coslp[k] = cos_ ( tl );
      dxbd += ccpamv[k] * ( sinlp[k] + pecc * sin_ ( pomg ) );
      dybd += - ccpamv[k] * ( coslp[k] + pecc * cos_ ( pomg ) );
      dzbd += - ccpamv[k] * sorbel[k+13] * cos_ ( plon - sorbel[k+5] );
   }
...
/* Heliocentric coordinates of the Earth */
   dr = dpsi * d1pdro;
   flatm = ccim * sin_ ( forbel[2] );
   a = sigma * cos_ ( flatm );
   dxh = dr * dcosls - a * coslm;
   dyh = dr * dsinls - a * sinlm;
   dzh = - sigma * sin_ ( flatm );
...
/* Barycentric coordinates of the Earth */
   ...
   dzb = dzh * dc1mme;
   for ( k = 0; k < 4; k++ ) {
      flat = sorbel[k+13] * sin_ ( forbel[k+3] - sorbel[k+5] );
      a = ccpam[k] * (1.0 - sorbel[k+9] * cos_ ( forbel[k+3] - sorbel[k+1]));
      b = a * cos_(flat);
      dxb -= b * coslp[k];
      dyb -= b * sinlp[k];
      dzb -= a * sin_ ( flat );
