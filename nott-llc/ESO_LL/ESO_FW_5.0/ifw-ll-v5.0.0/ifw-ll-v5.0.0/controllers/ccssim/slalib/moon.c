...
/* Longitude */
   ...
   for ( n = NL -1; n >= 0; n-- ) {
      ...
      v += coeff * ( (float) sin_ ( (double) theta ) );
      dv += coeff * ( (float) cos_ ( (double) theta ) ) *
...
/* Parallax */
   ...
   for ( n = NP - 1; n >= 0; n-- ) {
      ...
      fn = (float) tp[n].nf;
      theta = emn * em + empn * emp + dn * d + fn * f;
      v += coeff * ( (float) cos_ ( (double) theta ) );
      dv += coeff * ( - (float) sin_ ( (double) theta ) ) *
                    ( emn * em1 + empn * emp1 + dn * d1 + fn * f1 );
   }
   p = D2R * v;
   dp = RATCON * dv;
...
/* Parallax to distance (AU, AU/sec) */
   sp = (float) sin_ ( (double) p );
   r = ERADAU / sp;
   dr = - r * dp * (float) ( cos_ ( (double) p ) ) / sp;

/* Longitude, latitude to x, y, z (AU) */
   sel = (float) sin_ ( (double) el );
   cel = (float) cos_ ( (double) el );
   sb = (float) sin_ ( (double) b );
   cb = (float) cos_ ( (double) b );
   rcb = r * cb;
   rbd = r * db;
   w = rbd * sb - cb * dr;
   x = rcb * cel;
...
/* Mean obliquity */
   ...
   sineps = (float) sin_ ( (double) eps );
   coseps = (float) cos_ ( (double) eps );
