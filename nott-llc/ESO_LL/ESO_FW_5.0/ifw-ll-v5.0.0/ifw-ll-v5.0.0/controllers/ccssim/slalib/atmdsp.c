...
#include "slalib.h"
#include "slamac.h"
void slaAtmdsp ( double tk, double phpa, double rh, double wl1,
...
**  - - - - - - - - - -
**
**  Apply atmospheric-dispersion adjustments to optical/IR refraction
**  coefficients.
**
**  Given:
**     tk    double   ambient temperature, K
**     phpa  double   ambient pressure, hPa (= millibars)
**     rh    double   ambient relative humidity, 0-1
**     wl1   double   reference wavelength, micron (0.4 recommended)
...
**
**  2  Most of the atmospheric dispersion happens between 0.7 microns and
**     the UV atmospheric cutoff, and the effect increases strongly
**     towards the UV end.  For this reason a blue reference wavelength
**     is recommended, for example 0.4 microns.
**
...
**          reference wavelength    0.4 microns
**                star elevation    15 deg
**
**     is about 2.5 mas RMS between 0.3 and 1.0 microns, and stays
**     within 4 mas for the whole range longward of 0.3 microns
**     (compared with a total dispersion from 0.3 to 20.0 microns of
**     about 11 arcsec).  These errors are typical for ordinary
...
**
**  4  If either wavelength exceeds 100 microns (or is negative, used in
**     in certain other functions to indicate frequency), the non-
**     dispersive low-frequency radio case is assumed and the returned
**     refraction coefficients are the same as the given ones.  Note
**     that radio refraction coefficients cannot be turned into optical
**     values using this function, nor vice versa.
**
...
*/
{
   double tkok, phpaok, rhok, psat, pwo, w1, wlok, wlsq, w2, dn1, dn2, f;


/* Check for radio. */
   if ( wl1 < 0.0 || wl2 < 0.0 || wl1 > 100.0 || wl2 > 100.0 ) {

   /* Radio: assume no dispersion. */
      *a2 = a1;
      *b2 = b1;
   ...
   /* Optical: keep arguments within safe bounds. */
      tkok = gmax ( tk, 100.0 );
      tkok = gmin ( tkok, 500.0 );
      phpaok = gmax ( phpa, 0.0 );
      phpaok = gmin ( phpaok, 10000.0 );
      rhok  = gmax ( rh, 0.0 );
      rhok  = gmin ( rhok, 1.0 );

   /* Atmosphere parameters at the observer. */
      psat = pow_ ( 10.0, -8.7115 + 0.03477 * tkok );
      pwo = rhok * psat;
      w1 = 11.2684e-6 * pwo;
