...
*/
{
  ...
  x2 = xi * xi;
  y2 = eta * eta;
  sd = (float) sin_ ( (double) dec );
  cd = (float) cos_ ( (double) dec );
  sdf = sd * (float) sqrt_ ( (double) ( 1.0f + x2 + y2 ) );
  r2 = cd * cd * ( 1.0f + y2 ) - sd * sd * x2;
  if ( r2 >= 0.0f ) {
     r = (float) sqrt_ ( (double) r2 );
     s = sdf - eta * r;
     c = sdf * eta + r;
     if ( xi == 0.0f && r == 0.0f ) {
        r = 1.0f;
     }
     *raz1 = slaRanorm ( ra - (float) atan2_ ( (double) xi, (double) r ) );
     *decz1 = (float) atan2_ ( (double) s, (double) c );
     r = -r;
     s = sdf - eta * r;
     c = sdf * eta + r;
     *raz2 = slaRanorm ( ra - (float) atan2_ ( (double) xi, (double) r ) );
     *decz2 = (float) atan2_ ( (double) s, (double) c );
     *n = ( fabs_ ( (double) sdf ) < 1.0 ) ? 1 : 2;
  } else {
     *n = 0;
  }
}
