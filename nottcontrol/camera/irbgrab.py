...
def getDLLHandle(irbgrab_path = None):    
    ...
    if 'win' in os.name or 'nt' in os.name:
        ...
        if not irbgrab_path.is_file():
            libdir = pl.Path(r"C:\Program Files (x86)\InfraTec\IRBGRAB_SDK\v4")
            irbgrab_path = libdir / 'irbgrab_win64.dll'
...
#        p_search_string=ct.c_char_p(bytes(search_string,'ascii'), size = CHAR_BUFFER_SIZE)
    ...
    #Allgemein, struct (ct) und param_type müssen selbst erstellt werden
    ...
    def get_data(self,img_type):
        ...
        if img_type==1: #für Bitmap Daten muss IRBG_PARAM_SDK_NeedBitmap32 gesetzt sein 
    ...
    def get_digital_values(self):
        minimum = ct.c_uint16(0)
        maximum = ct.c_uint16(0)
        
        res = self.irbgrab_dll.irbgrab_mem_getdigitalvalues(self._img_pointer,
                                                            ct.byref(minimum),
                                                            ct.byref(maximum))
        res=res&MASK
        if res==hirb.IRBG_RET_SUCCESS: return (hex(res), minimum.value, maximum.value)
        else: return (hex(res),)
    
    def get_timestamp(self):
        timestamp = ct.c_double()
        
        res = self.irbgrab_dll.irbgrab_mem_gettimestamp(self._img_pointer,
                                                        ct.byref(timestamp))
        
        res=res&MASK
        if res==hirb.IRBG_RET_SUCCESS: return (hex(res), timestamp)
        else: return (hex(res),)
...
#        img_pointer=ct.POINTER(ct.c_int64(self._img_pointer))
    ...
    def get_data_easy(self,img_type): #eventuell prüfen ob vorheriges Bild bearbeitet wurde
    ...
    def get_data_easy_noFree(self,img_type): #eventuell prüfen ob vorheriges Bild bearbeitet wurde
        res=self.get_data(img_type)
        if int(res,16)==hirb.IRBG_RET_SUCCESS:
            dim=self.get_dimensions()
            if int(dim[0],16)==hirb.IRBG_RET_SUCCESS:
...
def test():
    ...
    print('connecting to camera: {}'.format(irbgrab.connect('')))
